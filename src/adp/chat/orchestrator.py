...
ADP-914.8: run_turn's optional `diagram_context` parameter extends this to
the diagram editor (web/src/diagrams/DiagramEditorPage.tsx) without adding
any new adp.chat.tools.TOOL_REGISTRY entry -- the frontend supplies the
diagram's current title/type/DSL directly (works uniformly for saved and
ADP-914.7-generated-but-unsaved diagrams alike, since neither a tool call
nor `diagram_context` itself ever needs a persisted diagram_id), appended to
that turn's system prompt via `_DIAGRAM_EDIT_INSTRUCTIONS` below the same
way `context_block` already grounds capability/application answers. See
specs/049-ai-diagram-editing/research.md Decision 1 for the full reasoning,
including why a new tool was considered and rejected.
"""
...
# ADP-914.8: appended after _SYSTEM_PROMPT (or its admin-overridden
# equivalent, ADP-SPEC-042) only when diagram_context is present. Fixed
# text, not admin-editable independently -- it's a mechanical convention the
# frontend's extractProposedDsl() (web/src/diagrams/editor/) depends on
# matching exactly, not a tunable persona/tone choice like the base prompt.
_DIAGRAM_EDIT_INSTRUCTIONS = (
    "\n\nYou are also currently viewing a diagram (its title, type, and "
    "current DSL source are given below). Answer questions about it using "
    "only that content -- never invent nodes, edges, or structure not "
    "actually present. If asked to change the diagram, respond with the "
    "complete updated DSL for the whole diagram (not a diff) inside a "
    "single fenced code block, using the diagram's type as the code "
    "block's language tag, and nothing else DSL-shaped outside that block. "
    "If a request is too ambiguous to propose a concrete change, ask a "
...
async def run_turn(
    *,
    ...
    `diagram_context` (ADP-914.8): when the caller is the diagram editor
    (DiagramEditorPage.tsx), the frontend supplies the diagram's own current
    title/type/DSL here on every turn -- appended to the system prompt
    alongside `_DIAGRAM_EDIT_INSTRUCTIONS`, exactly the way `context_block`
    below already grounds capability/application answers. Deliberately NOT a
    new adp.chat.tools.TOOL_REGISTRY entry (research.md Decision 1): a tool
    the model calls needs an id to call it *with*, but a diagram generated by
    ADP-914.7 and not yet saved has none -- the frontend already holds the
    full content in memory regardless of save state, so it supplies it
    directly rather than the model fetching it. `None` (every existing,
    non-diagram caller) leaves system-prompt assembly byte-for-byte
    ...
    with ai_step_span("chat_turn", operation_id=conversation_id) as span:
        ...
        if diagram_context is not None:
            system_prompt += f"{_DIAGRAM_EDIT_INSTRUCTIONS}\n\n{diagram_context}"
