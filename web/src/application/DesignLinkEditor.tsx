...
export default function DesignLinkEditor({ appId }: Props) {
  ...
  return (
    <div>
      <h4 style={{ margin: "0 0 8px", fontSize: 13, fontWeight: 600, color: "#444" }}>Linked Designs</h4>
      {toast && <div style={{ fontSize: 11, color: "#c00", marginBottom: 6 }}>{toast}</div>}
      {links?.items.length === 0 && <div style={{ fontSize: 12, color: "#888", marginBottom: 8 }}>No designs linked.</div>}
      {links?.items.map(link => {
        ...
        return (
          <div key={link.design_id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <button onClick={() => deleteLink.mutate(link.design_id)} style={{ fontSize: 11, color: "#c00", background: "none", border: "none", cursor: "pointer" }}>✕</button>
          </div>
        );
      })}
      {available.length > 0 && (
        <div style={{ display: "flex", gap: 6, marginTop: 8, alignItems: "center" }}>
          <select value={selectedDesignId} onChange={e => setSelectedDesignId(e.target.value)} style={{ flex: 1, fontSize: 12, padding: "4px 6px", border: "1px solid #ccc", borderRadius: 4 }}>
