...
export default function TechCapLinkEditor({ appId }: Props) {
  ...
  return (
    <div>
      <h4 style={{ margin: "0 0 8px", fontSize: 13, fontWeight: 600, color: "#444" }}>Technical Capabilities</h4>
      ...
      {["provides", "consumes"].map(ut => {
        ...
        return (
          <div key={ut} style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#666", textTransform: "uppercase", marginBottom: 3 }}>{ut}</div>
            {group.length === 0 && <div style={{ fontSize: 11, color: "#aaa" }}>None</div>}
            {group.map(link => (
              <div key={`${link.tech_cap_id}-${link.usage_type}`} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                ...
                <button onClick={() => deleteLink.mutate({ tcId: link.tech_cap_id, usageType: link.usage_type })} style={{ fontSize: 11, color: "#c00", background: "none", border: "none", cursor: "pointer" }}>✕</button>
              </div>
            ))}
          </div>
        );
      ...
      {(caps?.items.length ?? 0) > 0 && (
        <div style={{ display: "flex", gap: 6, marginTop: 8, alignItems: "center" }}>
          <select value={selectedTcId} onChange={e => setSelectedTcId(e.target.value)} style={{ flex: 1, fontSize: 12, padding: "4px 6px", border: "1px solid #ccc", borderRadius: 4 }}>
            <option value="">— Tech capability —</option>
            {caps?.items.map(c => <option key={c.id} value={c.id}>{c.name} (L{c.level})</option>)}
          </select>
          <select value={usageType} onChange={e => setUsageType(e.target.value as UsageType)} style={{ fontSize: 12, padding: "4px 6px", border: "1px solid #ccc", borderRadius: 4 }}>
