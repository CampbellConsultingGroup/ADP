    ...
    <div>
      <div style={{ marginBottom: 10 }}>
        ...
        <input
          ...
          style={{ width: "100%", padding: 8, fontSize: 13, border: "1px solid var(--border)", borderRadius: 4, boxSizing: "border-box", background: "var(--surface)", color: "var(--ink)" }}
        />
        {statement.length > 0 && statement.trim().length < 10 && (
          <div style={{ fontSize: 12, color: "var(--crit)", marginTop: 4 }}>Minimum 10 characters required.</div>
        )}
      </div>
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 4 }}>Kind *</label>
        <select value={kind} onChange={(e) => setKind(e.target.value as RequirementKind)} style={{ padding: "7px 10px", fontSize: 13, border: "1px solid var(--border)", borderRadius: 4, background: "var(--surface)", color: "var(--ink)" }}>
      ...
      <button
        ...
        style={{ padding: "8px 18px", background: canSubmit ? "var(--good)" : "var(--border)", color: "var(--surface)", border: "none", borderRadius: 4, cursor: canSubmit ? "pointer" : "not-allowed", fontSize: 14, fontWeight: 600 }}
      >
        {add.isPending ? "Adding..." : "Add Requirement"}
      </button>
      {successMsg && <div style={{ marginTop: 8, color: "var(--good)", fontSize: 13, fontWeight: 600 }}>{successMsg}</div>}
      {add.isError && <div style={{ marginTop: 8, color: "var(--crit)", fontSize: 13 }}>Failed to add requirement.</div>}
    </div>
  );
}
