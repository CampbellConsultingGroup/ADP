...
export default function AcceptDialog({ option, onConfirm, onCancel, isPending }: AcceptDialogProps): React.ReactElement {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "32px 16px", boxSizing: "border-box" }}>
      {/* maxHeight + flex column + overflow hidden on the frame, with only the
          middle section scrolling: options with many proposed elements were
          taller than the viewport and had no scroll container anywhere, so
          "Confirm Accept" rendered off-screen with no way to reach it. */}
      <div style={{ background: "var(--surface)", borderRadius: 8, padding: 24, maxWidth: 500, width: "90%", maxHeight: "100%", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 17, fontWeight: 700, flexShrink: 0 }}>Accept Recommendation</h3>
