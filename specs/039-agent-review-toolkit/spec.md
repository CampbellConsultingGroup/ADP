...
- **ART-IX** — Provenance and Auditability: an accepted suggestion MUST write an `AuditEntry` with `origin="ai"`, the confirming human as actor, and MUST carry provenance back to the review operation that produced it (mirroring how an accepted recommendation option carries `provenance=option_id`).
...
2. **Given** a pending `set_maturity_level` suggestion, **When** the architect accepts it, **Then** the capability's `maturity_level` updates to the suggested value, an `AuditEntry` with `origin="ai"` and the architect as actor is written, and the suggestion cannot be accepted a second time.
...
- **Stale reference at accept time**: the capability, domain, or other cited entity a suggestion references is edited or deleted between suggestion generation and human review → accept MUST re-verify current state immediately before writing and fail explicitly (not silently apply against stale data) if it no longer matches.
...
- **FR-003**: System MUST provide shared helpers for writing the audit entry (`origin="ai"`, confirming human as actor, provenance to the suggestion/operation) and any reasoning-trail record produced when a suggestion is accepted, so each adapter does not re-derive this logic.
...
- **FR-015**: Accepting a suggestion MUST re-verify the current state of every entity it references immediately before writing, and MUST fail explicitly (without writing) if that state no longer matches what the suggestion was generated against.
...
- **AuditEntry (existing)**: reused for every accepted suggestion's write, with `origin="ai"`.
...
- **SC-002**: 100% of accepted suggestions produce exactly one `AuditEntry` with `origin="ai"` and the confirming human as actor, written via the same store functions a manual edit would use — verified by there being no code path that writes capability/domain data other than the existing manual-edit path plus this one, shared, review-accept path.
