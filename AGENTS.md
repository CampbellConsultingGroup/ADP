...
Latest work: **048-generate-diagrams-from-data** (ADP-914.7, implemented — both user stories) — "Generate Diagram" buttons on `ValueStreamDetail.tsx` and `CapabilityNode.tsx` produce a new, unsaved flowchart pre-filled from ADP's own business data: a value stream's ordered stages, or a capability's full subtree. New `web/src/diagrams/generators.ts` — two pure functions building a typed `DiagramModel` via the vendored `diagram-core`'s `addNode`/`addEdge`, never hand-written DSL text. One-way only by design (FR-008) — a saved generated diagram is indistinguishable from a hand-authored one, no provenance, no re-sync. Cross-page hand-off (Business page → Diagrams page) deliberately reuses `App.tsx`'s existing `currentDesignId`/`onSelectDesign` lifted-state pattern rather than a new mechanism. Two design corrections caught during implementation: `CapabilityNode.tsx`'s prop had to widen from `BusinessCapability` to `CapabilityTreeNode` to reach the full subtree (the plan's initial "just read `capability.children`" assumption was wrong — confirmed by direct code read, not assumed); `addNode()` auto-generates node ids internally, so the value-stream generator builds an id-map while creating nodes (the capability generator needs none — its top-down recursion has the parent id on hand via closure). Zero backend change. 151 frontend tests (was 141, +10), `tsc --noEmit` clean, zero regressions. See `specs/048-generate-diagrams-from-data/`.
