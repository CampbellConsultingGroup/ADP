...
Latest work: **047-persona-diagram-experience** (ADP-914.6, implemented — both user stories) — new diagrams get a persona-aware default type (Enterprise Architect → `architecture`, Solution Architect → `flowchart`, Technical Architect → `sequence`) and a "(Recommended for your role)" label on the matching option in `DiagramEditorPage.tsx`'s type selector — steering only, `WRITE_DIAGRAM` untouched. Also corrects a stale wording gap found while scoping this feature: ADP-914's epic said "EA/BA/TA" but ADP has no `business_architect` `PersonaRole` at all — spec.md fixes the wording rather than inventing a fourth role. Pure frontend, zero backend change: new `web/src/diagrams/persona.ts` (a static constant mirroring `AuthProvider.tsx`'s `ROLE_LABELS` pattern), confirmed via direct read that `AuthProvider` blocks all children until `role` resolves so there's no mount-time race to design around. Found and fixed one bug beyond what the task list literally named: `model`'s state initializer had the same hardcoded `"flowchart"` fallback as `diagramType` and would have gone out of sync with it — both now derive from one shared value. Bundled in the same branch: **ADP-914.5**, the nav-wiring fix for 046's diagram editor, which had been fully built and tested but was never reachable from the running app at all (no `AppView`/nav entry existed) — new `DiagramsPage.tsx` (mirrors `ApplicationPage.tsx`'s list↔detail convention) now wired into `App.tsx`/`AppShell.tsx` as a "Diagrams" nav item. 141 frontend tests (was 124, +17 across both fixes), `tsc --noEmit` clean, zero regressions. See `specs/047-persona-diagram-experience/`.
