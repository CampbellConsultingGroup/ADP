...
Latest work: **050-strategic-objective-capture** (ADP-d8u.1, implemented — all three user stories) — a new sibling package `src/adp/strategy/` captures `StrategicTheme`/`StrategicObjective` as structured entities (owner, statement, an all-or-nothing metric/target/unit/direction group, a structured fiscal_year+period horizon) rather than a text blob, with many-to-many links to real `business_capabilities`/`value_streams` (never free text). New migration 025 mirrors migration 008's `capability_design_links` join-table shape exactly (composite PK, `ON DELETE CASCADE` both legs, one index, `created_at`); the cascade was verified live against real Postgres, zero orphaned join rows after deleting an objective. Cross-package link validation calls `adp.business.store.get_capability`/`get_value_stream` through a *second*, business-scoped session (mirrors `adp.business.router`'s own `_get_application_session` pattern). Write endpoints reuse the existing `WRITE_BUSINESS_ARCH` action via a new `/api/v1/strategy/` prefix rule — no new `ActionType`, no inline per-route permission helper (mirrors `adp.diagrams.router`'s simpler idiom). Frontend: `web/src/strategy/` — list/form/detail screens mirroring `DomainList`/`ValueStreamList`'s convention, plus two link editors that are near-verbatim mirrors of `DesignLinkEditor.tsx`, wired into a new "Strategy" nav entry placed first under Architecture. 1185 backend tests (+38), 192 frontend tests (+23), `ruff`/`mypy`/`tsc` clean — plus a full live Playwright walkthrough (create → link → edit → delete) against real Postgres. See `specs/050-strategic-objective-capture/`. Filed as a follow-on: ADP-d8u.2, designs also need to link to strategic objectives, mirroring the same capability/value-stream link pattern a third time.
