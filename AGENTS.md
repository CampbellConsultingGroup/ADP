...
Latest work: **ADP-914.14** (canvas-v2 browser E2E coverage, implemented — small task bead, no `specs/` dir) — closes the gap ADP-914.13 left when it deleted `tests/e2e/workspace.spec.ts` alongside `web/src/canvas/`. New `web/tests/e2e/canvas-v2.spec.ts` + a `playwright.config.ts` "canvas-v2" project, covering level toggle, add-element, and select-and-inspect against a real backend/DB, no mocking — deliberately not reviving the deleted spec's mocked-API/direct-URL-navigation approach, since `App.tsx` has no router at all (confirmed via `package.json`: no `react-router` dependency) and `page.goto("${WEB_URL}/designs/...")` never actually worked. **A genuine, pre-existing bug found while validating the new spec against `flows.spec.ts`'s established "fullstack" pattern**: that file had the identical broken assumption (`page.goto` immediately followed by asserting a "Designs" heading, no click in between) across 5 of its own tests — fixed all 5 in the same pass, plus `exact: true` once `OverviewPage`'s redesigned dashboard tiles started substring-matching the same locator. Two further classes of `flows.spec.ts` staleness were found but deliberately not chased (same substring-ambiguity pattern elsewhere, plus at least two renamed buttons) — filed as `ADP-914.16` instead of expanding scope. 249 frontend tests unchanged, `tsc` clean, all 5 new tests plus the 27 `api` tests confirmed passing live against a real local stack.

Prior work: **ADP-914.13** (retire `web/src/canvas/`, implemented) — Phase C of the C4Canvas-retirement roadmap: deleted `web/src/canvas/` wholesale (every symbol confirmed zero-upstream-caller via GitNexus `impact()`), moved `c4-filter.ts` to `canvas-v2/` rather than deleting it since `c4Adapter.ts` shares it, removed `usePlaceElement`/`useDrawRelationship`/`notifyConflict` as dead code tied to the deletion, retired the legacy "Canvas" nav entry (`C4DesignView` is now the sole design-scoped surface), and repointed `RecommendationPage.tsx`'s redirect. 1231 backend tests unchanged, 249 frontend tests (−8 for the deleted test file), full roadmap ADP-914.9/.11/.12/.13 closed. See git history for the full writeup.
