...
Latest work: **049-ai-diagram-editing** (ADP-914.8, implemented — both user stories) — `DiagramEditorPage.tsx` embeds `adp.chat`'s existing `ChatButton`/`ChatPanel` (already used on the Capabilities page): architects can ask grounded questions about the diagram currently open (including unsaved content, US1), or request an edit that's applied directly to the live, reviewable editor state for review (US2) — the *existing* Save button remains the sole persistence gate, no new accept/reject UI. First AI-generative capability in this diagram line (046/914.6/914.7 all had zero AI-generated content). **Zero new `adp.chat.tools.TOOL_REGISTRY` entries** — planning ruled out a `get_diagram(diagram_id)` tool because it can't cover an ADP-914.7-generated, not-yet-saved diagram (no id for a tool to look up); instead the frontend supplies diagram content directly via a new optional `diagram_context` field on `SendMessageRequest`, appended into `run_turn`'s system prompt the same way its existing hybrid-search `context_block` already grounds capability/application answers — `adp.chat`'s mechanically-enforced read-only tool boundary test needed zero changes. A proposed edit is a fenced DSL code block in the assistant's streamed reply, detected client-side once streaming completes and applied via the *existing* `applyDsl()`. Manual editing locks while a request is in flight (Clarifications, FR-011), reusing this file's own `disabled={saving}` idiom; the large vendored `Canvas.tsx` got a non-invasive wrapping lockout rather than a threaded prop, keeping it a low-diff mirror. 1138 backend tests (+2), 169 frontend tests (+14), `ruff`/`mypy`/`tsc` clean. See `specs/049-ai-diagram-editing/`.
