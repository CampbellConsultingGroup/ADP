...
Latest work: **ADP-914.13** (retire `web/src/canvas/`, implemented — a small task bead, no `specs/` dir) — Phase C of the C4Canvas-retirement roadmap, closing it out once ADP-914.12 (054-c4-design-view) merged and the user confirmed today's implementation + live walkthrough was validation enough to proceed. Deleted `web/src/canvas/` wholesale (`C4Canvas.tsx`, `Workspace.tsx`, `ConflictNotification.tsx`, `edges/`, `nodes/`) — every symbol confirmed zero-upstream-caller via GitNexus `impact()` before deletion. One real catch from that pass: `c4-filter.ts`'s level-filter functions were *shared* between the legacy view and `canvas-v2/c4Adapter.ts` (054's own adapter reuses it unchanged) — deleting it per the bead's original pre-054 file list would have broken the feature just shipped, so it was **moved** to `web/src/canvas-v2/c4-filter.ts` instead of deleted, test moving alongside it. Also removed as dead code tied directly to the deletion: `usePlaceElement`/`useDrawRelationship`/`notifyConflict` (the two hooks whose broken whole-design-`PUT` was ADP-914.1–.4's root cause, now fully superseded by ADP-SPEC-054's granular endpoints), their associated types, and their now-pointless test files (including a compile-time-only type test with no component left to check). Found but deliberately left alone as out-of-scope: `theme/c4-theme.ts`'s `getElementStyle` turned out to be *pre-existing* orphaned dead code, never actually wired to the deleted node component despite appearances — flagged as a separate follow-up, not fixed here. Nav: removed the legacy "Canvas" entry/`AppView` value entirely; `C4DesignView` is now the sole design-scoped fallback; `RecommendationPage.tsx`'s post-accept redirect repointed at it; dropped the "(Preview)" qualifier from its label/breadcrumb/subtitle since it's no longer contrasted against an alternative. Removed `playwright.config.ts`'s "browser" e2e project (only ever targeted the deleted `workspace.spec.ts`) rather than repointing it — new browser-level e2e coverage for `canvas-v2` is a real, explicitly-filed gap. 1231 backend tests unchanged, 249 frontend tests (was 257, −8 for the deleted C4Canvas test file), `tsc`/`adp-generate --check` clean, GitNexus `detect_changes()` confirmed the touched-symbol set matched intent before committing, plus a live Playwright walkthrough confirming the nav/breadcrumb/page all read "C4 Design" with existing design data intact. The full roadmap (ADP-914.9/.11/.12/.13) is now closed.

Prior work: **054-c4-design-view** (ADP-914.12, implemented — all four user stories) — Phase B of the C4Canvas-retirement roadmap decided on ADP-914.9: a genuinely new editing surface for the *canonical* `ArchitectureDescription`/`Element`/`Relationship` model, built on the diagram tool's reused `Canvas.tsx`/`DslPanel.tsx` (ADP-SPEC-052) instead of ReactFlow. Two new pieces make the vendored editor speak the canonical model: `web/src/canvas-v2/c4Adapter.ts` (pure `Element[]`/`Relationship[]` ⇄ `DiagramModel` mapping) and `web/src/canvas-v2/reconcile.ts` (diffs `Canvas.tsx`'s single whole-model `onChange` against the previous model and fires the one specific granular mutation that matches — commits immediately per action). A documented UX limitation, not a bug: `Canvas.tsx`'s toolbar exposes only 4 generic shapes with no way to set a node's `role`, so a toolbar-added element's kind is inferred from a fixed 1:1 shape convention (person↔circle, system↔rectangle, container↔rounded-rectangle, component↔diamond). New backend `src/adp/api/routers/elements.py`, 5 granular endpoints — **the actual fix for ADP-914.1–.4**, whose root cause was `usePlaceElement`/`useDrawRelationship` calling a whole-design `PUT` that was never registered as a route. ID generation uses max(existing)+1 (not `len+1`), since this is the first feature to add deletion and a naive counter would collide after a gap. No new tables — export wires straight to the existing locked-theme render/CALM endpoints, preserving ADP-SPEC-010's fixed-visual-identity guarantee. 1231 backend tests (was 1190, +41), 257 frontend tests (was 228, +29), `ruff`/`mypy`/`tsc`/`adp-generate --check` all clean. See `specs/054-c4-design-view/`.
...
This project is indexed by GitNexus as **ADP** (14035 symbols, 22278 relationships, 232 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
