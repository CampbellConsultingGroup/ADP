...
Latest work: **ADP-914.15** (dead-code cleanup, implemented — small task bead, no `specs/` dir) — pure deletion, no behavior change. Re-verifying the bead's own dead-code claim at pickup time (as its description explicitly asked) turned up something bigger than expected: not just `getElementStyle`/`C4ElementStyle` in `web/src/theme/c4-theme.ts`, but the **entire `web/src/theme/` directory** had zero consumers anywhere outside its own test file. Deleted both files plus `web/tests/unit/c4-theme.test.ts`; removed `C4ElementStyle` from `types.ts`. **`C4Theme` (a different, still-alive type) was explicitly kept** — confirmed `web/src/api/theme.ts`'s `useC4Theme()` genuinely uses it, and that `"./theme"` imports elsewhere (`ui/AppShell.tsx`) resolve to the unrelated `ui/theme.ts` light/dark-mode module, not `src/theme/` — three different "theme" modules exist in this codebase, worth spelling out since it'd be easy to conflate them. 244 frontend tests (was 249, −5), `tsc` clean, GitNexus `detect_changes()` confirmed low risk.
