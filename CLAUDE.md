...
Auto-generated from all feature plans. Last updated: 2026-08-11 (041-ai-chat-assistant plan added)
...
## Recent Changes
- 047-persona-diagram-experience: Implemented (ADP-914.6, both user stories) — new diagrams get a persona-aware default type (Enterprise Architect → `architecture`, Solution Architect → `flowchart`, Technical Architect → `sequence`) and a "(Recommended for your role)" label on the matching option in `DiagramEditorPage.tsx`'s type selector — steering only, never a restriction (`WRITE_DIAGRAM` untouched). Also closes a gap found while scoping this feature: ADP-914's epic wording said "EA/BA/TA" but ADP has no `business_architect` `PersonaRole` at all (confirmed via `src/adp/authz/roles.py`) — spec.md corrects the stale wording rather than inventing a fourth role. Pure frontend, zero backend change: new `web/src/diagrams/persona.ts` (a static `PERSONA_DEFAULT_TYPE` constant, mirroring `AuthProvider.tsx`'s `ROLE_LABELS` pattern) reused from a single point inside `DiagramEditorPage.tsx`'s existing `newDiagramType ?? "flowchart"` fallback, confirmed via direct read that `AuthProvider` blocks all children until `role` resolves so there's no mount-time race to design around. Found and fixed one bug during implementation, beyond what the task list literally named: the `model` state's initializer had the same hardcoded `"flowchart"` fallback as `diagramType` and would have gone out of sync with it (e.g. `diagramType="sequence"` paired with an empty *flowchart* model) — both now derive from one shared `initialType`. Bundled in the same branch: ADP-914.5, the nav-wiring fix for ADP-SPEC-046's diagram editor, which had been fully built and tested but was never reachable from the running app (no `AppView`/nav entry) — new `DiagramsPage.tsx` owns list↔editor state (mirrors `ApplicationPage.tsx`'s convention) and is wired into `App.tsx`/`AppShell.tsx`/`shell/index.ts` as a new "Diagrams" nav item. 141 frontend tests (was 124 pre-ADP-914.5, +17 across both fixes), `tsc --noEmit` clean, zero regressions.
- 046-diagram-type-support: Implemented (ADP-SPEC-046, all 3 user stories) — five new standalone diagram types (flowchart, sequence, ER, UML, cloud-architecture) additive alongside the existing C4 workspace; zero changes to `ArchitectureDescription`, `web/src/canvas/` (C4), or `adp.renderer`. Reuses a sibling project's mature TypeScript diagramming library (`/home/jmuir/projects/canvas/packages/diagram-core`, discovered and read directly during planning — not assumed from its description), **vendored** (copied, not npm-linked) into `web/src/diagrams/core/` rather than a live cross-repo dependency, specifically so the build stays reproducible from a clean ADP checkout alone (ART-XIV). Parsing, DSL validation, and SVG rendering run entirely client-side; the new `adp.diagrams` backend package is deliberately just CRUD storage of an opaque, size-capped `dsl_source` string, plus one PNG-export endpoint reusing the `cairosvg` dependency ADP's C4 pipeline already has (the sibling project's own PNG path uses a Node-only binding, not portable to Python). One new standalone table (`diagrams`, migration 024, no FK to `designs` — a deliberate stepping stone toward a future optional-Design-attachment model, chosen specifically to make that a clean additive migration rather than one requiring rework). New `ActionType.WRITE_DIAGRAM` (`PERMISSIONS_VERSION` 1.7.0 → 1.8.0), granted to the three architect roles, denied to Reviewer — extended the existing completeness-checked `tests/authz/test_permissions.py` tables in place rather than adding an isolated test file, since that file already asserts every `ActionType`/`PersonaRole` combination is enumerated. Verified security property, not assumed: the vendored SVG renderer's `escapeXml()` was directly confirmed (not just described) to escape all user-supplied text. 1136 backend tests (+50), 124 frontend tests across 21 files (was ~108), `ruff`/`mypy`/`tsc` clean, `adp-generate --check` clean. Found and documented two pre-existing, out-of-scope gaps along the way: `web/` has no `eslint.config.js` at all (empty git history), and a real inaccuracy in this feature's own draft quickstart.md (a C4 test-filter scenario that matched zero files, since `web/src/canvas/` has no dedicated tests of its own) — fixed in-spec rather than left to bite a future session.
- 045-application-export: Implemented (ADP-SPEC-045, ADP-81p.2, both user stories) — extends ADP-SPEC-044's continuous-export pattern to the Application registry (ADP-81p's second and largest remaining domain): applications, technical capabilities, transformation initiatives (with member/disposition links), and application-to-application integrations, all reconciled to one versioned JSON file per entity under `$ADP_BUSINESS_ARCH_EXPORT_ROOT/applications/`, sharing the exact same env vars/background-task lifecycle as 044 — no new configuration surface. New shared `adp.export.common` module (research.md Decision 5) extracted from `adp.export.business_arch`'s domain-agnostic mechanics (path safety, atomic writes, content-diff writes, orphan cleanup, background-loop lifecycle) — a behavior-preserving refactor validated by 044's own existing test suite passing unchanged, zero test-file edits. Per Clarification Q1 (explicit, recorded trade-off — not a default), the export includes an application's risk/cost/governance records **unredacted**, even though the live API gates them behind `READ_APPLICATION_{RISK,COST,GOVERNANCE}`: a background export process has no per-viewer permission context to apply that gate selectively, so the safer, honestly-documented choice was full inclusion with an explicit residual-risk callout (Threat Model, RUNBOOK.md) rather than silent partial redaction. New database read pattern: bulk-list store functions reused where they exist, direct `Table` queries (joined for display names) where they don't — same precedent 044 set for `value_stream_stages`. No new database table, no new API endpoint — the exported file tree is this feature's external interface too, documented as a sibling contract to 044's. 1086 backend tests (+30 from this feature), 2 export-cycle integration tests (business architecture + application registry, both against real Postgres), `ruff`/`mypy` clean, `adp-generate --check` clean.
...
This project is indexed by GitNexus as **ADP** (12770 symbols, 20433 relationships, 235 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
