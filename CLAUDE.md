...
Auto-generated from all feature plans. Last updated: 2026-08-10 (041-ai-chat-assistant plan added)
...
- 044-business-arch-export: Implemented (ADP-SPEC-044, ADP-81p.1, both user stories) — continuous, opt-in export of business capabilities/value streams/stages/domains from Postgres to one versioned JSON file per entity, closing the "architecture as code" gap for this domain (ADP-81p). New `adp.export.business_arch` module: a periodic full-reconciliation background task (not write-path hooks — `value_stream_stages` has no `updated_at` column at all, ruling that out) wired into `adp.api.app`'s existing lifespan, inert unless `ADP_BUSINESS_ARCH_EXPORT_ROOT` is set. No new database table — change detection compares candidate content against what's already on disk (content-based, ignoring the file's own `exported_at` field), and deletion cleanup scans each output directory's filenames against the live ID set; a deleted value stream's whole directory (its stages included) is removed in one step. No new API endpoint — the exported file tree itself is the feature's external interface, documented as a contract. Scope deliberately excludes applications and the two design-linking join tables (those connect to entities that are out of scope or separately exported); the one in-scope cross-entity relationship, which capabilities a value stream stage links to, is included since omitting it would leave the export nearly meaningless. 1039 backend tests, 3 integration tests (real Postgres), `ruff`/`mypy` clean, `adp-generate --check` clean.
...
This project is indexed by GitNexus as **ADP** (12014 symbols, 19005 relationships, 214 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
