...
Auto-generated from all feature plans. Last updated: 2026-08-05 (041-ai-chat-assistant plan added)
...
- 042-admin-prompt-management: Implemented (ADP-SPEC-042, ADP-t32, all 3 user stories) — an admin-only screen to view/edit/confirm/revert the system prompts behind ADP's six AI agent call sites without a code deploy. New `PersonaRole.PLATFORM_ADMIN` + `ActionType.MANAGE_AGENT_PROMPTS` (closes a wildcard-grant gap: `ENTERPRISE_ARCHITECT`'s `frozenset(ActionType)` grant is narrowed so no architect role auto-inherits the new action; `ADPAdministrator` Keycloak group remapped from `ENTERPRISE_ARCHITECT` to the new role); two new additive tables (`agent_prompt_overrides`, `agent_prompt_history` via migration 023) sit in front of the existing hardcoded prompt constants as an override layer (constants remain the fallback, read via a new `adp.admin.prompt_registry` module — deliberately not `adp.agents`, whose ADP-SPEC-039 boundary test forbids importing any single domain module); reuses the existing `confirmation_id` human-confirmation pattern identically for both edits and restores; new `web/src/admin/` page gated at the route level, distinct from every existing page (per Clarification: no architect role, including Enterprise Architect, gets access by virtue of that role alone). 1015 backend tests, 108 web tests, `ruff`/`mypy`/`tsc` clean, production build succeeds.
...
This project is indexed by GitNexus as **ADP** (11798 symbols, 18717 relationships, 208 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
