...
- 048-generate-diagrams-from-data: Implemented (ADP-914.7, both user stories) — "Generate Diagram" buttons on `ValueStreamDetail.tsx` and `CapabilityNode.tsx` produce a new, unsaved flowchart pre-filled from ADP's own business data: a value stream's ordered stages (sequential edges) or a capability's full subtree (parent→child edges). New `web/src/diagrams/generators.ts`, two pure `(source data) -> DiagramSeed` functions, both building a typed `DiagramModel` via the vendored `diagram-core`'s `addNode`/`addEdge` — never hand-written DSL text; the existing `useDslSync` machinery derives the DSL panel automatically, same as any manual edit. One-way only, by design (FR-008) — a saved generated diagram is stored identically to a hand-authored one, no provenance kept, no re-sync. Cross-page hand-off (Business page → Diagrams page/editor) deliberately reuses `App.tsx`'s **existing** `currentDesignId`/`onSelectDesign` lifted-state pattern (a new `pendingDiagramSeed`/`onGenerateDiagram`) rather than inventing a new state-sharing mechanism. Zero backend change. Two design corrections found and applied during implementation: `CapabilityNode.tsx`'s `capability` prop had to widen from `BusinessCapability` to `CapabilityTreeNode` to reach the full subtree (confirmed the plan's naive assumption — "just read `capability.children`" — was wrong, since the prop was flat); `addNode()` auto-generates each node's id internally, so `generateFromValueStream` builds a `stageId → generatedNodeId` map while creating nodes (`generateFromCapabilitySubtree` needs no such map — its top-down recursion always has the parent's id on hand via closure). 151 frontend tests (was 141, +10), `tsc --noEmit` clean, zero regressions.
...
This project is indexed by GitNexus as **ADP** (13058 symbols, 20842 relationships, 236 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
