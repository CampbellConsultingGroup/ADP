...
## Recent Changes
- 051-strategy-landing-card: Implemented (ADP-d8u.3, all three user stories) — a fifth "Strategy" domain card on `OverviewPage.tsx`, closing the open-frontier gap where Strategy (ADP-d8u.1) had zero landing-dashboard presence while Business/Enterprise/Solution/Technical already did. Unlike the other four cards (whose mini-stats are all derived client-side from list data the page already fetches), Strategy's linkage-health split and fiscal-period breakdown need per-objective link facts the existing `GET /objectives` list response doesn't carry, and the fiscal comparison must be anchored to the server's clock, not the browser's — so this adds `GET /api/v1/strategy/summary`, a new aggregate endpoint mirroring `adp.portfolio`'s own already-established `GET /api/v1/portfolio/summary` pattern (raw `sa.text()` SQL, same response-model shape, same frontend hook shape including `staleTime`) rather than inventing a new one. No migration — reads only the four already-existing `adp.strategy` tables. The one query computes all seven fields atomically in a single round trip, including a `CASE`-expressed rule for `FY`-period objectives (past due only once their whole fiscal year has elapsed, never partway through it) — verified correct by seeding objectives across every bucket against real Postgres, since the SQL uses `NOW()`/`EXTRACT()`/`FILTER` that the SQLite-backed unit tests can't execute (their tests mock `session.execute()` instead, mirroring `adp.portfolio`'s own established test pattern for this exact class of endpoint). Two real corrections found during implementation: the mocked-session test design only became apparent after discovering the SQLite incompatibility firsthand; and a **pre-existing** test file, `web/tests/component/overview-page.test.tsx` (outside `web/src/overview/`, missed by the initial premise check), broke because its route-mock map didn't include the new endpoint — fixed by adding it, zero other regressions. 1190 backend tests (was 1185, +5), 202 frontend tests (was 192, +10), `ruff`/`mypy`/`tsc`/`adp-generate --check` clean — plus a full live Playwright walkthrough seeding objectives across every governance-signal state (linked/unlinked, past-due/current/upcoming) and confirming the rendered card matches the live API exactly. See `specs/051-strategy-landing-card/`.
- 050-strategic-objective-capture: Implemented (ADP-d8u.1, all three user stories) — a new sibling package `src/adp/strategy/` (models/store/router) captures `StrategicTheme`/`StrategicObjective` as structured entities (owner, statement, an all-or-nothing metric/target/unit/direction group, a structured fiscal_year+period horizon) rather than a text blob, with many-to-many links out to real `business_capabilities`/`value_streams` — never free text. New migration 025: `strategic_themes`, `strategic_objectives`, and two join tables (`strategic_objective_capabilities`/`strategic_objective_value_streams`) mirroring migration 008's `capability_design_links` shape exactly (composite PK, `ON DELETE CASCADE` both legs, one index, `created_at`); verified live against real Postgres that the cascade leaves zero orphaned join rows. Cross-package link validation (research.md Decision 2) calls `adp.business.store.get_capability`/`get_value_stream` directly through a *second*, business-scoped session the router obtains via its own `_get_business_session` dependency — mirrors `adp.business.router`'s own `_get_application_session` pattern rather than `adp.chat`'s session-factory variant, since these are plain request-scoped endpoints, not a background task/SSE generator. Write endpoints reuse the existing `ActionType.WRITE_BUSINESS_ARCH` via a new `/api/v1/strategy/` entry in `adp.authz.enforcement`'s prefix-rule table (no inline per-route permission helper, matching `adp.diagrams.router`'s newer, simpler idiom over `adp.business.router`'s older inline-check one). Frontend: `web/src/strategy/` — `ThemeList`/`ObjectiveList`/`ObjectiveForm`/`ObjectiveDetail` (mirror `DomainList`/`ValueStreamList`'s list+inline-create-form convention) plus `ObjectiveCapabilityLinkEditor`/`ObjectiveValueStreamLinkEditor`, near-verbatim mirrors of `DesignLinkEditor.tsx` (research.md Decision 4) — except the "linked" set comes straight off the loaded `StrategicObjective`'s `capability_ids`/`value_stream_ids` rather than a separate query, since `GET /objectives/{id}` already returns them inline. New "Strategy" nav entry (`shell/index.ts`/`AppShell.tsx`/`App.tsx`), placed first under Architecture ahead of Business per explicit request. One deliberate v1 simplification worth knowing: `StrategicObjectiveUpdate`'s metric-group validator only enforces all-or-nothing among the fields *present in that PATCH* — a caller changing just `target_value` on an objective that already has a full metric group must resupply all four fields in the same request, or it's rejected as partial (no fetch-then-merge check against the existing row). 1185 backend tests (was 1147, +38), 192 frontend tests (was 169, +23), `ruff`/`mypy`/`tsc` clean, `adp-generate --check` clean — plus a full live Playwright walkthrough (create theme → create objective with metric group → link capability → edit → delete-with-confirm) against real Postgres, not just the automated suite. See `specs/050-strategic-objective-capture/`. Filed as a follow-on: ADP-d8u.2 (designs also need to link to strategic objectives, mirroring the capability/value-stream link pattern a third time).
- 049-ai-diagram-editing: Implemented (ADP-914.8, both user stories) — `DiagramEditorPage.tsx` embeds `adp.chat`'s existing `ChatButton`/`ChatPanel` (already used on the Capabilities page): US1 lets an architect ask grounded questions about the diagram currently open (including unsaved content); US2 lets them request an edit that's applied directly to the live, reviewable editor state (Canvas + DSL panel) for review, with the *existing* Save button remaining the sole persistence gate — no new accept/reject UI. First AI-*generative* capability in this diagram feature line (046/914.6/914.7 all had zero AI-generated content). **Zero new `adp.chat.tools.TOOL_REGISTRY` entries** — planning ruled out a `get_diagram(diagram_id)` tool (the initially-plausible design) because it can't cover an ADP-914.7-generated, not-yet-saved diagram, which has no id for a tool to look up; instead the frontend supplies the diagram's title/type/DSL directly via a new optional `diagram_context` field on `SendMessageRequest`, appended into `run_turn`'s system prompt the same way its existing `context_block` already grounds capability/application answers — so `adp.chat`'s mechanically-enforced read-only tool boundary (`tests/unit/chat/test_tools_boundary.py`) needed zero changes. A proposed edit is a fenced DSL code block in the assistant's plain-text streamed reply, detected client-side (`extractProposedDsl.ts`, a new pure function) once streaming completes and applied via the *existing* `applyDsl()` — the same mechanism already used when reopening a saved diagram. Manual editing is locked while a request is in flight (Clarifications, FR-011) via a new `onStreamingChange` callback, reusing this file's own pre-existing `disabled={saving}` idiom; `DslPanel.tsx` (small, ADP-authored) got a real `disabled` prop, while `Canvas.tsx` (the large vendored+adapted component) got a non-invasive wrapping lockout instead of a threaded prop, keeping it a low-diff mirror per this project's vendoring convention. 1138 backend tests (+2), 169 frontend tests (+14), `ruff`/`mypy`/`tsc` clean, `adp-generate --check` clean.
...
This project is indexed by GitNexus as **ADP** (13533 symbols, 21589 relationships, 233 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
