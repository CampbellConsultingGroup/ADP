...
Auto-generated from all feature plans. Last updated: 2026-08-13 (918-strategy-rollups implemented)
...
- 917-objective-design-traceability: Implemented (ADP-d8u.2, both user stories) — closes the top-priority open-frontier traceability gap named in the source requirements doc: objectives previously linked only to capabilities/value streams (Layer 1); this adds forward links to the designs (Layer 3, `objective_design_links`) and applications (Layer 2, `objective_application_links`) that actually realize them, both directions surfaced. **Six ground-truth corrections against the source doc**, all verified via direct migration/code reads before writing the spec: every id in this schema is a plain string, not the doc's assumed UUID (`strategic_objectives.id` is `String(36)`, `designs.id` is unbounded `Text`/`DSN-NNN`, `applications.id` is `String(36)`); `GET /store/designs/{id}/objectives` named a nonexistent `adp.store` router — the real endpoint landed in the already-existing `src/adp/api/routers/designs.py` under `/api/v1/designs`; the applications router prefix is plural (`/api/v1/applications`), not the doc's singular guess; no new package or submodule was needed — extends the existing `adp.strategy` `models.py`/`store.py`/`router.py` directly (1,596 lines pre-feature, comfortably under the ~2,847-line split threshold, and this is "more of the same" shape as the two existing capability/value-stream link tables, not a new concept the way ADP-d8u.6's initiatives were); design/application existence checks reuse `adp.business.store`'s own established lightweight-read-only-table-mirror pattern (`_designs`/`_applications` declared inside `adp.strategy.store`'s own `_metadata`) rather than the heavier `DesignStore.get()`/full application fetch — and since these mirrors sit in the same physical Postgres database, no second cross-package session was even needed for them (a **self-caught simplification** over the original plan.md/data-model.md design, which had over-specified a `_get_store_session` mirroring `_get_business_session`'s pattern before realizing the physical-database identity made it unnecessary); `web/src/designs/` has no single-design detail screen at all (`DesignsPage.tsx` is a flat list, `onSelectDesign` still routes to Intake) — resolved via a real `AskUserQuestion` clarification to place the reverse "Objectives realizing this design" panel inside `C4DesignView.tsx` (the only design-scoped screen that exists today) as an explicitly-flagged placeholder location. A second real clarification resolved linking as intentionally unconstrained — no soft consistency check requiring a design/application to already be reachable via the objective's own capability/value-stream links. 1355 backend tests (was 1327, +28), 279 frontend tests (was 265, +14), `ruff`/`mypy`/`tsc`/`adp-generate --check` all clean — plus a full live Playwright walkthrough (linking a design and an application to an objective from the objective side, confirming both reverse panels on the C4 Design View's new Traceability section and the Application detail's new Objectives Realized tab, and a direct DB check confirming zero orphaned link rows after deleting the objective) against a running local stack, test data cleaned up afterward. See `specs/917-objective-design-traceability/`.
...
This project is indexed by GitNexus as **ADP** (14904 symbols, 24057 relationships, 225 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
